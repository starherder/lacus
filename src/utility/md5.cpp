...
char* PrintMD5(uchar digest[16], char* buffer, size_t size)
{
	assert(size > 32);

	safe_sprintf(buffer, size,
...
char* MD5String(const char* szString, char* buffer, size_t size)
{
	...
	return PrintMD5(alg.Digest(), buffer, size);

}

// Performs the MD5 algorithm on a file (binar or text),
// returning the results as a char*.  Returns nullptr if it fails.
char* MD5File(const char* szFilename, char* buffer, size_t size)
{
	FILE* file;
	md5 alg;
	size_t nLen;
	...
	try
	{
		...
		if ((file = fopen(szFilename, "rb")) != nullptr)
		{
			//while (nLen = fread(chBuffer, 1, 1024, file))
			while ((nLen = fread(chBuffer, 1, 1024, file)) > 0)
			{
